...
namespace AWEX12000
{
    ...
    public class AlarmSystem
    {
        ...
        private List<ArmMtrl> ArmTable; //Alarm Table(自定義的Alarm表格(含模組)，由Alarm.xls讀取而來)
        public List<ArmMtrl> ArmUIList; //由Timer從List中取出給UI顯示用
        private Queue<ArmMtrl> ArmList; //存放由Say函式來的Alarm (待處理佇列，處理後的資料會顯示在UI)
        ...
        //+ By Kent 20250313 防短時間內的AlarmLog記錄
        private bool IsStopDelayRecord;
        private Thread thDelayRecord;
        public Queue<ArmMtrl> DelayRecordAlarm; //防短時間內重覆記錄
        ...
        public AlarmSystem()
        {
            ...
            DelayRecordAlarm = new Queue<ArmMtrl>();
            ...
            IsStopDelayRecord = false;
            thDelayRecord = new Thread(funcDelayRecord);
            thDelayRecord.IsBackground = true;
            thDelayRecord.Start();
        }
        ...
        private bool GetRepeatInDelayRecordList(ArmMtrl pArm)
        {
            bool bRet = DelayRecordAlarm.Any(x => (x.AlarmLevel == pArm.AlarmLevel) && (x.ModuleID == pArm.ModuleID) && (x.ErrorCode == pArm.ErrorCode) && (x.Explain == pArm.Explain));
            return bRet;
        }

        private bool GetRepeatInShowList(ArmMtrl pArm)
        {
            //ArmMtrl arm = ArmUIList.fin(x =>
            //    (
            //    (x.AlarmLevel == pArm.AlarmLevel) &&
            //    (x.ModuleID == pArm.ModuleID) &&
            //    (x.ErrorCode == pArm.ErrorCode) 
            //    ));
            ...
            //{
            //    // 用正則表達式將數字部分替換掉
            //    string pattern = @"\d+"; // 匹配數字
            //    string normalizedStr1 = Regex.Replace(arm.Explain, pattern, "###");
            //    string normalizedStr2 = Regex.Replace(pArm.Explain, pattern, "###");
        ...
        private void funcDelayRecord()
        {
            while (!IsStopDelayRecord)
            {
                if (SYSPara.HMIReady)
                {
                    UInt64 tm = Convert.ToUInt64(SYSPara.SetupReadValue("AlarmLog寫入延遲時間").ToInt());
                    if (SYSPara.SingleRunTM > tm)
                    {
                        if (DelayRecordAlarm.Count > 0)
                        {
                            lock (lockObj)
                            {
                                ArmMtrl pArm = DelayRecordAlarm.Dequeue();
                                //計算Alarm的持續時間並記錄到Log(無用)
                                AddAlarmToLog(pArm);
                            }
                        }
                    }
        ...
        private void AddAlarmToLog(ArmMtrl pArm)
        {
            ...
            try
            {
                ...
                using (FileStream fs = new FileStream(tempfile, FileMode.Open, FileAccess.Read))
                {
                    ...
                    for (int j = 2; j <= sheet.LastRowNum; j++)
                    {
                        ...
                        FieldSet(row, 8, ref pArm.Lan3rd.Explain);
                        FieldSet(row, 9, ref pArm.Lan3rd.Cause);
                        FieldSet(row, 10, ref pArm.Lan3rd.Solution);

                        FieldSet(row, 11, ref pArm.mLightSet.Red);
                        FieldSet(row, 12, ref pArm.mLightSet.RedBlink);
                        FieldSet(row, 13, ref pArm.mLightSet.Yellow);
                        FieldSet(row, 14, ref pArm.mLightSet.YellowBlink);
                        FieldSet(row, 15, ref pArm.mLightSet.Green);
                        FieldSet(row, 16, ref pArm.mLightSet.GreenBlink);
                        FieldSet(row, 17, ref pArm.mLightSet.Blue);
                        FieldSet(row, 18, ref pArm.mLightSet.BlueBlink);
                        FieldSet(row, 19, ref pArm.mLightSet.MusicNo);
                        FieldSet(row, 20, ref pArm.MTBF);
                        FieldSet(row, 21, ref pArm.MTBA);
                        FieldSet(row, 22, ref pArm.SplitStatistic);
                        ArmTable.Add(pArm);
                    }
                    ...
                    if (String.Compare(ErrCode, "ClearAll") != 0)
                    {
                        ...
                        if (armMtrlTemp != null)
                        {
                            if (Language == "tw")
                                ...
                                aArm.Explain = armMtrlTemp.Lan3rd.Explain;
                    ...
                    if (String.Compare(ErrCode, "ClearAll") != 0)
                    {
                        ...
                        if (armMtrlTemp != null)
                        {
                            ...
                            if (Language == "tw")
                                ...
                                explain = armMtrlTemp.Lan3rd.Explain;
                            if (args.Count() > 0)
        ...
        public bool ClearAll()
        {
            ...
            {                
                foreach (ArmMtrl pArm in ArmUIList)
                {
                    ...
                    {
                        //防短時間記錄功能只在生產模式使用
                        if (!GetRepeatInDelayRecordList(pArm))
                            DelayRecordAlarm.Enqueue(pArm);
                    }
                    else
                    {
                        //計算Alarm的持續時間並記錄到Log(無用)
