...
namespace PaeLibBibExchangerModule
{
    ...
    public class BemElevator : JModuleBase, IDisposable
    {
        ...
        /// 前板定位偵測(B接點)靠近手推台車
        /// </summary>
        private DigitalInput DI_FrontBIBPositionDetector = null;

        /// <summary>
        /// 後板定位偵測(B接點)靠近主機
        /// </summary>
        private DigitalInput DI_BackBIBPositionDetector = null;

        /// <summary>
        /// Z軸馬達
        /// </summary>
        private Motor MO_AxisZ = null;
        ...
        /// <summary>
        /// 讀取推車板子高度偏移量(相對於進出板高度而言)
        /// </summary>
        public int CarrierBIBReadHeightOffset { get; set; }

        /// <summary>
        /// 讀取推車高度偏移量(相對於進出板高度而言)
        /// </summary>
        public int CarrierReadHeightOffset { get; set; }
            ...
            : base(alarm)
        {
            ...
            CarrierBIBReadHeightOffset = 0;
            CarrierReadHeightOffset = 0;
        }
        ...
        /// <summary>
        /// 移動至指定平台之指定樓層高度進行推車板讀取
        /// </summary>
        /// <param name="stageID">指定平台ID : 下層平台為 0，上層平台為 1</param>
        /// <param name="slotHeight">指定樓層高度(相對於第一層)</param>
        /// <returns>true : 移動完成，false : 移動中...</returns>
        public bool GotoCarrierBIBReadPosition(int stageID, int slotHeight)
        {
            int DetectBIBHeight = slotHeight + CarrierBIBReadHeightOffset;
            bool bRet = GotoCarrierWorkingPosition(stageID, DetectBIBHeight);
            return bRet;
        }
        ...
        /// <param name="slotHeight">指定樓層高度(相對於第一層)</param>
        /// <returns>true : 移動完成，false : 移動中...</returns>
        public bool GotoCarrierReadPosition(int stageID, int slotHeight)
        {
            int DetectBIBHeight = slotHeight + CarrierReadHeightOffset;
            bool bRet = GotoCarrierWorkingPosition(stageID, DetectBIBHeight);
            return bRet;
        }
