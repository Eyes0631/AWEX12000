...
namespace LPA
{
    ...
    public partial class LPA : BaseModuleInterface
    {
        ...
        private FlowChart.FCRESULT FC_Auto_Open_CalculationResult_Run()
        {
            ...
            if (OReadValue("ModuleTest").ToBoolean() == true || IsSimulation() > 0)
            {
                ...
                TrayDataSetting.tdFoup1.YN = 25;
                TrayDataSetting.tdFoup1.XN = 1;
            }
            else
            {
                FoupMapping.iScanWaferFirstTheoreticalPos = SReadValue("WaferFirstTheoreticalPos").ToInt();
                ...
                TrayDataSetting.tdFoup1.YN = PReadValue("Foup_YN").ToInt();
                TrayDataSetting.tdFoup1.XN = PReadValue("Foup_XN").ToInt();
            }
            ...
            if (OReadValue("DryRun").ToBoolean())
            {
                ...
                {
                    if (GetIsDummy() == false)
                        TrayDataSetting.tdFoup1.CellSet(0, 0, 0, i, (byte)_BinDefine.HasWafer, 0);
                }
            }
            else
            {
                for (int i = 0; i < waferState.Length; i++)
                {
                    switch (waferState[i])
                    {
                        case WaferState.NoWafer:
                            {
                                TrayDataSetting.tdFoup1.CellSet(0, 0, 0, i, (byte)_BinDefine.NoWafer, 0);
                            }
                            break;
                        ...
                        case WaferState.WaferDtected:
                            {
                                if (GetIsDummy() == false)
                                    ...
                                    TrayDataSetting.tdFoup1.CellSet(0, 0, 0, i, (byte)_BinDefine.DummyWafer, 0);
                            }
                            break;

                        case WaferState.ThicknessOverLimit:
                        case WaferState.WaferPosError:
                            TrayDataSetting.tdFoup1.CellSet(0, 0, 0, i, (byte)_BinDefine.WaferNG, 0);
