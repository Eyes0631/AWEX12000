...
namespace LPD
{
    ...
    public partial class LPD : BaseModuleInterface
    {
        ...
        private void PPFAProcess(object obj)
        {
            while (!StopDialogFormThread)
            {
                ...
                {
                    InputCassetteIDForm InputCstIDForm = new InputCassetteIDForm("FLP");
        ...
        private FlowChart.FCRESULT FC_Auto_Open_CalculationResult_Run()
        {
            ...
            if (OReadValue("ModuleTest").ToBoolean() == true || IsSimulation() > 0)
            {
                ...
                TrayDataSetting.tdFoup4.YN = 25;
                TrayDataSetting.tdFoup4.XN = 1;
            }
            else
            {
                FoupMapping.iScanWaferFirstTheoreticalPos = SReadValue("WaferFirstTheoreticalPos").ToInt();
                ...
                TrayDataSetting.tdFoup4.YN = PReadValue("Foup_YN").ToInt();
                TrayDataSetting.tdFoup4.XN = PReadValue("Foup_XN").ToInt();
            }
            ...
            if (OReadValue("DryRun").ToBoolean())
            {
                ...
                {
                    if (GetIsDummy() == false)
                        TrayDataSetting.tdFoup4.CellSet(0, 0, 0, i, (byte)_BinDefine.HasWafer, 0);
                }
            }
            else
            {
                for (int i = 0; i < waferState.Length; i++)
                {
                    switch (waferState[i])
                    {
                        case WaferState.NoWafer:
                            {
                                TrayDataSetting.tdFoup4.CellSet(0, 0, 0, i, (byte)_BinDefine.NoWafer, 0);
                            }
                            break;
                        ...
                        case WaferState.WaferDtected:
                            {
                                if (GetIsDummy() == false)
                                    ...
                                    TrayDataSetting.tdFoup4.CellSet(0, 0, 0, i, (byte)_BinDefine.DummyWafer, 0);
                            }
                            break;

                        case WaferState.ThicknessOverLimit:
                        case WaferState.WaferPosError:
                            TrayDataSetting.tdFoup4.CellSet(0, 0, 0, i, (byte)_BinDefine.WaferNG, 0);
