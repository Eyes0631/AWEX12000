...
namespace AWEX12000
{
    public partial class 內參設定 : BaseOptionF
    {
        ...
        private void cboxOEEReportSel_SelectedIndexChanged(object sender, EventArgs e)
        {
            SYSPara.OEERPFrom = (byte)dcbOeeSource.SelectedIndex;
        }

        private void ShowOeeType()
        {
            ...
            foreach (var file in di.GetFiles())
            {
                ...
                {
                    foreach (string item in dcbOeeDataType.Items)
                    {
                        if (!item.Contains(file.Name.TrimEnd(file.Extension.ToCharArray())))
                            continue;
                        else
                            isProductExist = true;
                    }
                    if (!isProductExist)
                    {
                        string nameProduct = file.Name.TrimEnd(file.Extension.ToCharArray());
                        dcbOeeDataType.Items.Add(nameProduct);
                    }
                }
            }
        }

        private void dcbOeeDataType_SelectedIndexChanged(object sender, EventArgs e)
        {
            SYSPara.OEERPType = dcbOeeDataType.Text;
        }
    }
}
