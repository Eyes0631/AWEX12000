...
namespace PaeLibGeneral
{
    ...
    public static class JLogger
    {
            ...
            bool fileNameWithDate = true, string fileName = "", string filePath = "")
        {
            ...
            if (fileNameWithDate)
            {
                ...
                if (level.Equals(LogLevel.Trace))
                    fileName = string.Format("{0}_{1}{2}", "Trace", DateTime.Now.ToString("yyyyMMdd"), en);
                else
        ...
        /// <summary>
        /// 紀錄 DEBUG Level 的 Exception Log
        /// </summary>
        /// <param name="ex">例外訊息物件</param>
        /// <param name="fileNameWithDate">Log檔名後方是否增加日期 "_YYYYMMDD"</param>
        /// <param name="fileName">Log指定檔名，預設檔名為 "loggerName.log"</param>
        /// <param name="filePath">Log指定路徑，預設路徑為 "執行檔路徑\App_Data\Log\"</param>
            ...
            bool fileNameWithDate = true, string fileName = "", string filePath = "")
        {
            ...
            string className = trace.GetFrame(0).GetMethod().ReflectedType.Name;
            string classFullName = trace.GetFrame(0).GetMethod().ReflectedType.FullName;
            int lineNumber = trace.GetFrame(0).GetFileLineNumber();
            int colNumber = trace.GetFrame(0).GetFileColumnNumber();
            string message = string.Format("In {0}, line:{1}-{2}, message:{3}", classFullName, lineNumber, colNumber, ex.Message);            
            LogEx(className, LogLevel.Debug, message, fileNameWithDate, fileName, filePath);
        }
