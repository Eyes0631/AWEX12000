...
{
    public class OCRParser_Cognex1741_CommandPort : OCRParser
    {
        ...
        public OCRParser_Cognex1741_CommandPort(string Name, eConnectedMode Mode, Unit unit = Unit.OCR)
        {
            m_Unit = unit;
            #region 避免程式重覆執行
            MutexName = Name;
        ...
        public bool CommnadTransferOperation(OCR_Command Cmd)
        {
            ...
            switch (Cmd.Command)
            {
                case CommDef.Login:
                    ...
                    Text = command + "\r\n";
                    break;
                case CommDef.LoadFile:
                    command = "LF" + Cmd.Parameters[0];
                    Text = command + "\r\n"; ;
                    break;
                //case CommDef.ReadBMP:
                //    command = "RB";
                //    Text = command + "\r\n"; ;
                    //break;
                case CommDef.SetOnline:
                    command = "SO" + Cmd.Parameters[0];
                    Text = command + "\r\n"; ;
                    break;
                case CommDef.GetOnline:
                    command = "GO";
                    Text = command + "\r\n"; ;
                    break;
                default:
        ...
        public override CommandParameter[] GetCmdParameterSetting(string Cmd)
        {
            ...
            if (Enum.TryParse(Cmd, out eCmd))
            {
                ...
                {
                    case CommDef.Login:
                        listCmdParas.Add(new CommandParameter("User", "admin"));
                        listCmdParas.Add(new CommandParameter("Password", ""));
                        break;
                    case CommDef.LoadFile:
                        listCmdParas.Add(new CommandParameter("Job", ""));
                        break;
                    //case CommDef.ReadBMP:
                    //    break;
                    case CommDef.SetOnline:
                        listCmdParas.Add(new CommandParameter("State", 0, 1));
                        break;
                    case CommDef.GetOnline:
                        break;
                    default:
