...
namespace AWEX12000
{
    public partial class MSS : Form
    {
        ...
        public void M_SetSpeed(int SpeedRate)
        {
            string modulename = "";
            try
            {
        ...
        private FlowChart.FCRESULT FC_AUTO_RESET_Run()
        {
            try
            {
                ...
                if (SYSPara.安全門偵測開關關閉)
                {
                    int iEQPSlowRunCycle = SYSPara.SetupReadValue("EQPSlowRunCycle").ToInt();
                    M_SetSpeed(iEQPSlowRunCycle); //降速
                    SYSPara.安全門偵測開關關閉 = false;
                }
            ...
            switch (SYSPara.SysState)
            {
                ...
                case StateMode.SM_PAUSE:
                    if (SYSPara.SysRun)
                    {
                        ...
                        {
                            int iEQPSlowRunCycle = SYSPara.SetupReadValue("EQPSlowRunCycle").ToInt();
                            M_SetSpeed(iEQPSlowRunCycle); //降速生產
                            SYSPara.安全門偵測開關關閉 = false;
                        }
                        SYSPara.SysState = StateMode.SM_AUTO_RUN;
                    }
        ...
        private FlowChart.FCRESULT flowChart4_Run()
        {
            ...
            switch (FormSet.mLotStartF.LotStartResult)
            {
                case ActionDT.ResultType.OK:
                    ...
                    OrgSysSpeedRate = SYSPara.OReadValue("機台速率").ToInt();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                bool old_安全門開關 = SYSPara.SetupReadValue("安全門偵測開關").ToBoolean();

                FormSet.m內參設定.RefreshAllData("Setup");
                FormSet.m內參設定.InitialSubPackage();
                FormSet.m內參設定.ShowDialog();

                bool new_安全門開關 = SYSPara.SetupReadValue("安全門偵測開關").ToBoolean();
                if ((old_安全門開關 != new_安全門開關) && (new_安全門開關))
                    M_SetSpeed(SYSPara.OrgSysSpeedRate); //恢復原速
            }
        }
        ...
        private void btnScanPPBody_Click(object sender, EventArgs e)
        {
            SYSPara.LogSay(EnLoggerType.EnLog_OP, "使用者按下「建立PPBody」");
            FormSet.mGemF.ScanAndCreatePPBody(FormSet.mPackage.PackageDS);
        }

        private void btnScanIOCheck_Click(object sender, EventArgs e)
        {
            var mNames = "";
            var isOk = true;
            foreach (BaseModuleInterface module in FormSet.ModuleList)
            {
                var scanResult = module.ScanIOCheckToFile();
                isOk &= scanResult;
                if (!scanResult)
                {
                    mNames += module.Name + "\n";
        ...
        private void chkboxCheckIO_CheckedChanged(object sender, EventArgs e)
        {
            var chkbox = sender as CheckBox;
            foreach (BaseModuleInterface module in FormSet.ModuleList)
            {
                module.CheckEnabled = chkbox.Checked;
            }
        }

        private async void btnRecordCylinderTime_Click(object sender, EventArgs e)
        {
            btnRecordCylinderTime.Enabled = false;
            await Task.Run(() =>
            {
                var moduleName = "MainFlow";
                try
                {
                    foreach (BaseModuleInterface module in FormSet.ModuleList)
                    {
                    ...
                    while (true)
                    {
                        ...
                        foreach (BaseModuleInterface module in FormSet.ModuleList)
                        {
                            ...
                            if (module.GetUseModule())
                            {
                                isRecordOk &= module.RecordCheckTime();
                            }
                        }
                        if (isRecordOk)
                            break;
                    ...
                    MessageBox.Show("記錄檢測時間完成", "訊息");
                }
                catch (Exception ex)
                {
                    SYSPara.Alarm.Say("W0603", moduleName);
                    MessageBox.Show(ex.StackTrace.ToString());
                }
            });
            btnRecordCylinderTime.Enabled = true;
        }

        private async void btnCheckCylinderTime_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("確認各氣缸已移至安全位置?", "訊息", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) != DialogResult.OK)
                return;
            btnCheckCylinderTime.Enabled = false;
            await Task.Run(() =>
            {
                var moduleName = "MainFlow";
                try
                {
                    ...
                    while (true)
                    {
                        var isCheckFinished = true;
                        foreach (BaseModuleInterface module in FormSet.ModuleList)
                        {
                            moduleName = module.Name;
                            if (module.GetUseModule())
                            {
                                isCheckFinished &= module.ManualIOCheck();
                            }
                        }
                        if (isCheckFinished)
                            break;
            ...
            btnCheckCylinderTime.Enabled = true;
        }

        private void tabMSS_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (tabMSS.SelectedTab == tpAutoIOCheck)
            {
                var x = 0;
                foreach (BaseModuleInterface module in FormSet.ModuleList)
                {
                    if (module.pnlAutoIOCheck.Controls.Count > 0)
                    {
                        module.pnlAutoIOCheck.Parent = tpAutoIOCheck;
                        module.pnlAutoIOCheck.Location = new Point(x, 100);
                        var lbl = new Label
                        {
                            Text = module.Name,
                            Location = new Point(x, 70)
                        };
                        tpAutoIOCheck.Controls.Add(lbl);
                        x += 200;
                    }
                }
            }
        ...
        public void DeselectAutoIOCheckTabPage()
        {
            tabMSS.DeselectTab("tpAutoIOCheck");
        }

        private void btn_FlowChartRecord_Click(object sender, EventArgs e)
        {
            FormSet.mMainFlow.FlowChartRecord();

            foreach (BaseModuleInterface module in FormSet.ModuleList)
            {
                if (module.Text == "MAA")
                    continue;
                SYSPara.CallProc(module.Text, "FlowChartRecord");
            }
        }
    }
}
