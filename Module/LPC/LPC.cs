...
namespace LPC
{
    ...
    public partial class LPC : BaseModuleInterface
    {
        ...
        private FlowChart.FCRESULT FC_Auto_Open_CalculationResult_Run()
        {
            ...
            if (OReadValue("ModuleTest").ToBoolean() == true || IsSimulation() > 0)
            {
                ...
                TrayDataSetting.tdFoup3.YN = 25;
                TrayDataSetting.tdFoup3.XN = 1;
            }
            else
            {
                FoupMapping.iScanWaferFirstTheoreticalPos = SReadValue("WaferFirstTheoreticalPos").ToInt();
                ...
                TrayDataSetting.tdFoup3.YN = PReadValue("Foup_YN").ToInt();
                TrayDataSetting.tdFoup3.XN = PReadValue("Foup_XN").ToInt();
            }
            ...
            if (OReadValue("DryRun").ToBoolean())
            {
                ...
                {
                    if (GetIsDummy() == false)
                        TrayDataSetting.tdFoup3.CellSet(0, 0, 0, i, (byte)_BinDefine.HasWafer, 0);
                }
            }
            else
            {
                for (int i = 0; i < waferState.Length; i++)
                {
                    switch (waferState[i])
                    {
                        case WaferState.NoWafer:
                            {
                                TrayDataSetting.tdFoup3.CellSet(0, 0, 0, i, (byte)_BinDefine.NoWafer, 0);
                            }
                            break;
                        ...
                        case WaferState.WaferDtected:
                            {
                                if (GetIsDummy() == false)
                                    ...
                                    TrayDataSetting.tdFoup3.CellSet(0, 0, 0, i, (byte)_BinDefine.DummyWafer, 0);
                            }
                            break;

                        case WaferState.ThicknessOverLimit:
                        case WaferState.WaferPosError:
                            TrayDataSetting.tdFoup3.CellSet(0, 0, 0, i, (byte)_BinDefine.WaferNG, 0);
