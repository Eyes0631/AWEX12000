...
namespace Cognex
{
    public class OCRParser_Cognex1741 : OCRParser
    {
        ...
        public OCRParser_Cognex1741(string Name, eConnectedMode Mode, Unit unit = Unit.OCR)
        {
            m_Unit = unit;
            #region 避免程式重覆執行
            MutexName = Name;
            ...
            if (Enum.TryParse(Cmd, out eCmd))
            {
                switch (eCmd)
                {
                    case CommDef.READ_CONFIG:
                        ...
                        break;
                    case CommDef.READ:
                        para = new string[] { "ProV_1", "ProV_2", "ProV_3" };
                        listCmdParas.Add(new CommandParameter("TEST", para));
                        break;
                    case CommDef.TUNE:
                        listCmdParas.Add(new CommandParameter("GG"));
                        listCmdParas.Add(new CommandParameter("Config", -1, 50));
                        para = new string[] { "ProV_1", "ProV_2", "ProV_3" };
                        listCmdParas.Add(new CommandParameter("TEST", para));
                        break;
