...
namespace AWEX12000
{
    public partial class HistoryLogger : Form
    {
        ...
        public HistoryLogger()
        {
            ...
            //+By Max 2019/09/11
            LoggerFactory.Logger.Add(null, EnLoggerType.EnLog_OP, "日期\t時間\t類型\t料號\t批號\t內容");
        ...
        private string oriMSG = string.Empty;
        private void AddMsg(string mType, string mMsg)
        {
            ...
            try
            {
                //+ By Stanley 20250326 For LogAnalyzer
                if (mType == "SPC")
                {
                    oriMSG = mMsg;
                    int idST = oriMSG.IndexOf("ST=");
                    oriMSG = oriMSG.Substring(idST, oriMSG.Length - idST);

                    mMsg = String.Format("{0}\t{1}\t{2}\t{3}\t{4}\t{5}",
                    DateTime.Now.ToString("yyyy/MM/dd"), DateTime.Now.ToString("HH:mm:ss.fff"), "OEE", SYSPara.PackageName, SYSPara.生產批號, oriMSG);                    
                }
        ...
        private void cycleWidget1_AfterSummary(object DataList)
        {
            try
            {
                ...
                OEE oee = new OEE();
                oee.Array = DataList;
                oee.OEERPType = SYSPara.OEERPType;
                oee.OEEFrom = SYSPara.OEERPFrom;

                string json = JsonConvert.SerializeObject(oee);

                ProcessStartInfo startInfo = new ProcessStartInfo
                {
                    FileName = Directory.GetCurrentDirectory() + "\\OEEperformance.exe",
                    Arguments = json
                };
    ...
    public class OEE
    {
        ...
        public string OEERPType
        {
            get { return type; }
            set { type = value; }
        }

        private short oeeFrom = 0;
        public short OEEFrom
        {
            get { return oeeFrom; }
            set { oeeFrom = value; }
        }
